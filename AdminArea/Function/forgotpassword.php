        ...
        if(isset($_POST['forgotPassword'])){
            if(empty($error)){
                ...
                if ($con -> connect_errno) { //Check it is the connection succesful
                    ...
                    $sql = "SELECT * from staff WHERE email='".$emailAddress."'";
                ...
                if (isset($email)){
                    ...
                    if (in_array($email, EMAIL_ALLOWED)){
                        ...
                        if ($con -> connect_errno) { //Check it is the connection succesful
                            ...
                            $sql = "SELECT * from staffresetpassword WHERE email='".$email."'";
                            if($result = $con->query($sql)){
                                if (($result->num_rows)>0){
                                    $result->free();
                                    $date = date('Y-m-d H:i:s');
                                    $sql = "UPDATE staffresetpassword SET resetCode = ?, request_time = ? WHERE email=?";
                                    ...
                                    $result->free();
                                    $date = date('Y-m-d H:i:s');
                                    $sql = "INSERT INTO staffresetpassword VALUES (?,?,?)";
                                    $code = uniqid("",true).uniqid("",true);
                                    $stmt = $con ->prepare($sql);
                        ...
                        try{
                            ...
                            $mail->Subject = 'Reset password for DailyMarket Website Admin Area account.';
                            $link = "AdminArea/resetpassword.php";
