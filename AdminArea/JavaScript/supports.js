/*Support Tickets Remove Confirmation box*/
...
var supportId = "";
function removeSupport(){ /*Remove support*/
    ...
    xmlhttp.open("GET", "AJAX/removeSupport.php?supportID=" + supportId+"&reason="+reason, true);
    xmlhttp.send();
    closeBox();
    return false;
}
function updateStatus(){ /*Update status and priority*/
    ...
    var priorityApply = document.getElementsByName("priority")[0].value;
    xmlhttp.onreadystatechange = function() {
      if (this.readyState == 4 && this.status == 200) {
        location.href="supports.php";
      }
