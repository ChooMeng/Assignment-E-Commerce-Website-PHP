...
function search(str,type,sort,sortOrder,page,total) {
    ...
    xmlhttp.open("GET", "AJAX/searchAudit.php?search=" + str+"&type="+type+"&sort='"+sort+"&sortOrder='"+sortOrder+"&page="+page+"&total="+total, true);
    xmlhttp.send();
}

/*Sort the table*/
function sort(col){
    var table = document.getElementById("auditList");
    var row = table.getElementsByClassName("auditData");
