/*Staff Remove Confirmation box*/
function openBox(staff){
...
var staffId = "";
function terminateStaff(){ //Remove staff account
    ...
    xmlhttp.open("GET", "AJAX/terminateStaff.php?staffID=" + staffId+"&reason="+reason, true);
    xmlhttp.send();
    closeBox();
    return false;
}
...
function search(str,type,sort,sortOrder,page,total) {
    ...
    xmlhttp.open("GET", "AJAX/searchStaff.php?search=" + str+"&status="+type+"&sort='"+sort+"&sortOrder='"+sortOrder+"&page="+page+"&total="+total, true);
    xmlhttp.send();
}
function sort(col){ //Sort the table
    var table = document.getElementById("staffList");
    var row = table.getElementsByClassName("staffData");
