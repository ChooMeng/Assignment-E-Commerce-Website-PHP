/*Client Remove Confirmation box*/
function openBox(client){
...
var clientId = "";
function terminateClient(){ //Remove client account
    ...
    xmlhttp.open("GET", "AJAX/terminateClient.php?clientID=" + clientId+"&reason="+reason, true);
    xmlhttp.send();
    closeBox();
    return false;
}
...
function search(str,type,sort,sortOrder,page,total) {
    ...
    xmlhttp.open("GET", "AJAX/searchClient.php?search=" + str+"&status="+type+"&sort='"+sort+"&sortOrder='"+sortOrder+"&page="+page+"&total="+total, true);
    xmlhttp.send();
}
