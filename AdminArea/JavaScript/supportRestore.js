...
function restoreSupport(){ /*Remove support*/
    var xmlhttp = new XMLHttpRequest();
    var reason = document.getElementsByName("reason")[0].value;
    xmlhttp.onreadystatechange = function() {
      if (this.readyState == 4 && this.status == 200) {
        location.href="supportRestore.php";
      }
    };
    xmlhttp.open("GET", "AJAX/restoreSupport.php?supportID=" + supportId+"&reason="+reason, true);
    xmlhttp.send();
    closeBox();
    return false;
}
...
function search(str,sort,sortOrder,page,total) {
    ...
    xmlhttp.open("GET", "AJAX/searchRemovedSupports.php?search=" + str+"&sort='"+sort+"&sortOrder='"+sortOrder+"&page="+page+"&total="+total, true);
    xmlhttp.send();
}
/*Sort the table*/
function sort(col){
