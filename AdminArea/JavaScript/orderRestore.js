...
function restoreOrder(){ /*Remove order*/
    var xmlhttp = new XMLHttpRequest();
    var reason = document.getElementsByName("reason")[0].value;
    xmlhttp.onreadystatechange = function() {
      if (this.readyState == 4 && this.status == 200) {
        location.href="orderRestore.php";
      }
    };
    xmlhttp.open("GET", "AJAX/restoreOrder.php?orderID=" + orderId+"&reason="+reason, true);
    xmlhttp.send();
    closeBox();
    return false;
}
...
function search(str,sort,sortOrder,page,total) {
    ...
    xmlhttp.open("GET", "AJAX/searchRemovedOrders.php?search=" + str+"&sort='"+sort+"&sortOrder='"+sortOrder+"&page="+page+"&total="+total, true);
    xmlhttp.send();
}
/*Sort the table*/
function sort(col){
