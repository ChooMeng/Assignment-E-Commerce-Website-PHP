                    ...
                    if($result = $con->query($sql)){
                        ...
                        if(!empty($existClientID)){
                            ...
                            if($result = $con->query($sql)){
                                ...
                                $sql = "UPDATE client SET clientID = ?, name = ?, gender = ?, birthDate = ?, contactNumber = ?, paymentMethod = ?,
                                        joinDate = ?, lastUpdate = ?, status = ?, defaultAddress = ? WHERE email = ?";
                                $stmt = $con->prepare($sql);
                                $stmt->bind_param('sssssssssss', $existClientID, $name, $gender, $birthDate, $contactNumber, $payment,
                                                $date, $date, $status, $addressID, $email);
                                if($stmt->execute()){
                                    $stmt->free_result();
                                    $result->free();
                                    $con->close();
                                    $_SESSION['guestId'] = $existClientID;
                            ...
                            if($result = $con->query($sql)){
                                ...
                                $addressID = $result->num_rows + 1;
                                $date = date('Y-m-d H:i:s');
                                $sql = "INSERT INTO client(clientID, name, gender, email, birthDate, contactNumber, paymentMethod,
                                        joinDate, lastUpdate, status, defaultAddress) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                                $stmt = $con->prepare($sql);
                                $stmt->bind_param('sssssssssss', $clientID, $name, $gender, $email, $birthDate, $contactNumber, $payment,
                                        $date, $date, $status, $addressID);
                                if($stmt->execute()){
                                    $stmt->free_result();                                    
                                    $sql = "INSERT INTO address(addressID, address, zipCode, city, state) VALUE (?, ?, ?, ?, ?)";
                                    $stmt = $con->prepare($sql);
                                    $stmt->bind_param('sssss', $addressID, $address, $zipCode, $city, $state);
                                    if($stmt->execute()){
                                        $stmt->free_result();
                                        $sql = "INSERT INTO clientaddress(clientID, addressID) VALUE (?, ?)";
                                        $stmt = $con->prepare($sql);
                                        $stmt->bind_param('ss', $clientID, $addressID);
                                        if($stmt->execute()){
                                            $stmt->free_result();
                                            $_SESSION['guestId'] = $clientID;
                                            $result->free();
                                            $con->close();
                                            header('HTTP/1.1 307 Temporary Redirect');
                                            header('Location: checkOut.php');
